...
void battle(int* flag, struct player* player, char* action);
int goblin(int* flag, struct player* player, int size);
...
int main() {
    ...
    while (flag != 2) {
        goblin(&flag, player, sizeof(&(player)->hero_name));
    }
    printf("\33[0d\33[2J");
    printf("\nGame over!\n\n");
    free(player);
